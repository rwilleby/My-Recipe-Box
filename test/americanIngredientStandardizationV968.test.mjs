...
assert.ok(rows.every(({ ingredient }) => ingredient.originalName), "approved card-transcribed names are preserved separately");
assert.ok(rows.every(({ ingredient }) => ingredient.originalUnit !== undefined), "approved card-transcribed units are preserved separately");
...
assert.ok(rows.every(({ recipeId, ingredient }) => /^AM-(?:00[1-9]|01\d|020)$/.test(recipeId) || ingredient.quantity === ingredient.qty));
...
assert.ok(rows.every(({ recipeId, ingredient }) => formatTextRecipeIngredient(ingredient).includes(
  /^AM-(?:00[1-9]|01\d|020)$/.test(recipeId) ? ingredient.recipeName : ingredient.originalName
...
assert.deepEqual(
  ...
  { canonicalName: "Beans — Kidney", unit: "ounce", packageSize: "15 ounces", preparation: "drained" },
);
