...
assert.ok(dietRecipes.filter((recipe) => Number(recipe.id.slice(3)) > 50).every((recipe) => recipe.ingredients.length === 4));
assert.ok(dietRecipes.filter((recipe) => Number(recipe.id.slice(3)) <= 50).every((recipe) => recipe.ingredients.length >= 8));
