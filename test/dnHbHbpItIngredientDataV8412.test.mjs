...
const expectedIngredientTotals = { DN: 100, HB: 337, HBP: 76, IT: 646 };
...
assert.equal(Object.values(expectedIngredientTotals).reduce((sum, count) => sum + count, 0), 1159);
...
assert.ok(byId["DN-001"].ingredients.some((item) => item.name === "Vegetable oil" && item.preparation === "as needed for frying"));
assert.ok(byId["DN-007"].ingredients.some((item) => item.name === "Fresh or frozen blueberries" && item.qty === 1));
