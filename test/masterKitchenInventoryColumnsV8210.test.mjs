...
for (const heading of ["Food", "Storage", "Form", "Package", "Have", "Buy", "Notes"]) {
  assert.match(page, new RegExp(`role="columnheader">${heading}`));
}
...
assert.match(styles, /\.masterInventoryLedger \{[^}]*grid-template-columns:[^}]*1\.65fr[^}]*\.85fr[^}]*1\.25fr[^}]*\.8fr[^}]*\.5fr[^}]*\.5fr[^}]*2\.8fr/s);
assert.match(styles, /text-transform: uppercase/);
assert.match(styles, /\.masterInventoryLedgerFamily h3 \{[^}]*font-family: Inter, Arial, sans-serif !important;[^}]*font-size: 12px !important/s);
assert.match(styles, /\.masterInventoryStorage \{[^}]*font-size: 8px/s);
assert.match(styles, /\.masterInventoryForm, \.masterInventoryPackage \{[^}]*font-size: 8px/s);
