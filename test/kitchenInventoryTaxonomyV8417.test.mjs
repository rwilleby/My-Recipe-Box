...
for (const variation of ["Raw Breast", "Raw Thigh", "Raw Bone-In Thigh", "Raw Drumstick", "Raw Wing", "Raw Leg Quarter", "Raw Whole Chicken", "Cooked Whole Breasts", "Cooked Diced Breast", "Cooked Sliced Breast", "Cooked Shredded Breast"]) assert.ok(chicken.some((item) => item.variation === variation));
...
assert.match(styles, /\.masterInventoryLedger \{[^}]*minmax\(110px, \.85fr\)/s, "desktop Unit column is widened");
...
assert.match(styles, /@media \(max-width: 1020px\)[\s\S]*grid-template-columns: minmax\(150px, 1\.3fr\) 105px 100px 132px 90px/s, "tablet Unit column remains usable");
