...
assert.match(component, /MEAL_BUILDER_POSITIONED_SIDE_TWO_IDS/);
...
assert.match(component, /position === "side-two"[^;]*MEAL_BUILDER_POSITIONED_SIDE_TWO_IDS\.has\(recipe\.id\)/);
assert.match(styles, /mealBuilderTrayFood-side-two\.is-positioned-overlay[^}]*top:\s*0[^}]*left:\s*0[^}]*width:\s*100%[^}]*height:\s*100%/);
assert.match(styles, /mealBuilderTrayFood-side-two\.is-positioned-overlay img[^}]*width:\s*100%[^}]*height:\s*100%[^}]*transform:\s*none/);
