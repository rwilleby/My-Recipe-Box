...
assert.match(maker, /\["favorites", "Custom"\]/);
assert.match(maker, /BULK_RECIPE_CODES = new Set\(\["SG", "CP", "CS"\]\)/);
assert.match(maker, /bulk: recipes\.filter\(isBulkRecipe\)/);
assert.match(maker, /function isBulkRecipe/);
assert.match(maker, /\^SG-\(\\d\{3\}\)\$/);
assert.match(maker, /Number\(match\[1\]\) >= 1 && Number\(match\[1\]\) <= 25/);
assert.doesNotMatch(maker, /form\.kind !== "bulk"/);
assert.match(maker, /IMAGE_KINDS.*complete.*healthy.*build/);
assert.match(maker, /IMAGE_KINDS.*bulk/);
...
assert.match(maker, /1 SERVING/);
assert.match(maker, /const facts = nutritionFor\(label\.nutrition \|\| \{ calories: label\.calories \}\)/);
assert.doesNotMatch(maker, /facts\.[a-zA-Z]+\s*\*\s*label\.(copies|amount|servings)/);
assert.match(maker, /<span>Frozen<\/span> <strong>/);
assert.match(maker, /<span>Use by<\/span> <strong>/);
...
assert.match(css, /border-bottom:1pt solid #1f2937!important/);
assert.match(css, /freezerImageLabelDates/);
assert.match(css, /margin-top:1\.1em!important/);
assert.match(css, /freezerImageLabelDates p\{font-weight:400!important/);
assert.match(css, /freezerImageLabelDates strong\{font-weight:800!important/);
...
assert.match(css, /grid-template-columns:1in minmax\(0,1fr\)/);
assert.match(css, /gap:\.125in/);
assert.match(css, /width:1\.3125in;height:\.775in/);
assert.match(css, /transform:translateX\(-\.125in\)/);
