...
assert.match(css, /\.mealBuilderWorkspaceGrid\s*\{[^}]*--meal-builder-selector-depth:[^;]*calc\(\(var\(--meal-builder-card-depth\) \* 2\) \+ 9px\)[^}]*grid-template-areas:\s*"heading \." "preview selectors"/);
...
assert.match(css, /\.mealBuilderMealSummaryCard\s*\{[^}]*height:\s*var\(--meal-builder-selector-depth\)[^}]*max-height:\s*var\(--meal-builder-selector-depth\)/);
...
assert.match(jsx, /mealBuilderWorkspaceGrid[\s\S]*mealBuilderSelectorsHeading[\s\S]*mealBuilderPreviewColumn[\s\S]*mealBuilderDishSelectors/);
