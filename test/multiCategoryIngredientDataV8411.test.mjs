...
const expectedIngredientTotals = { BR: 91, CC: 58, CO: 73, CP: 1498, CR: 67 };
...
assert.equal(Object.values(expectedIngredientTotals).reduce((sum, count) => sum + count, 0), 1787);
...
assert.ok(byId["BR-009"].ingredients.some((item) => item.name === "Egg yolk" && item.unit === "each" && item.preparation === "optional glossy finish"));
assert.ok(byId["CC-001"].ingredients.some((item) => item.name === "Large eggs" && item.qty === 2));
assert.ok(byId["CO-001"].ingredients.some((item) => item.name === "Apples" && item.qty === 6 && item.preparation === "peeled and sliced"));
...
assert.ok(byId["CR-001"].ingredients.some((item) => item.name === "Milk" && item.unit === "tablespoon" && item.recipeQuantityText === "2–3"));
