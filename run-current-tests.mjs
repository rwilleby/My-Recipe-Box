...
if (failures.length) {
  ...
  for (const failure of failures) {
    process.stderr.write(`\nFAILED: ${failure.file}\nExit status: ${failure.status ?? "none"}${failure.signal ? ` · Signal: ${failure.signal}` : ""}\n`);
    if (failure.error) process.stderr.write(`Runner error:\n${failure.error}\n`);
    if (failure.stdout) process.stderr.write(`Captured stdout:\n${failure.stdout}\n`);
    if (failure.stderr) process.stderr.write(`Captured stderr:\n${failure.stderr}\n`);
    if (!failure.error && !failure.stdout && !failure.stderr) process.stderr.write("The failed process returned no diagnostic output.\n");
  }
