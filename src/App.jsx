...
const { HomeComboMealStrip, HomeDietMealStrip } = createHomeMealRotations({
  DinnerCombinationImage,
  ...
  return (
    <>
      ...
      <HomeCategoryGrid
        ...
        SupplementalHoverVideo={SupplementalHoverVideo}
      />
      <HomeRecipeCounters
        recipes={recipes}
...
function ShoppingListPage({ plan, setPlan, checked, setChecked, servings, pantry, refrigerator, freezer, masterInventory, setMasterInventory, setActivePage, openRecipeCard, preparedInventory, preparedReservations, componentDecisions, setComponentDecisions, shoppingComments, setShoppingComments, shoppingOrderQuantities, setShoppingOrderQuantities, kosUi }) {
  ...
  const shoppingStockItems = useMemo(() => list.map((item) => {
  ...
  }, [plan, recipeById, dinnerCombinationById]); const shoppingPlannedMealCount = shoppingPlannedWeeks.flatMap((week) => week.meals).filter((meal) => meal.heroRecipeId || meal.heroMealId).length;
                  ...
                  <div className={`freezerPackagingSteps${accordion.illustrated ? "" : " textOnly"}`} aria-label={`Helpful guidance for ${accordion.title.toLowerCase()}`}>
                    {accordion.steps.map((step, index) => (
                      <article className={`freezerPackagingStep${accordion.illustrated ? "" : " freezerPackagingTextStep"}`} key={step.image || step.title}>
                        ...
                        <h3>{step.title}</h3>
                        <p>{step.copy}</p>
                        {index < accordion.steps.length - 1 && <i className="freezerPackagingArrow" aria-hidden="true">→</i>}
                      </article>
                    ))}
                  </div>
                </div>
...
function HiddenAboutRecipeBoxPage({ setActivePage }) {
  return (
    <main className="pageShell aboutRecipeBoxPage">
      <section className="aboutRecipeBoxIntro">
        <div>
          ...
          <p>
            ...
            is designed to help with the complete process—from choosing recipes and planning your
            week to shopping, preparing, portioning, freezing, storing, reheating, and serving.
          </p>
        </div>
        <blockquote>Cook once. Eat better all week.</blockquote>
      </section>

      <MealJourneyContent setActivePage={setActivePage} />

      <section className="aboutDifferenceSection" aria-labelledby="about-difference-title">
        <header className="mealJourneySectionHeader">
      ...
      <section className="aboutRobertSection">
        ...
        <div>
          <p className="mealJourneyEyebrow">THE PERSON BEHIND THE SITE</p>
          <h2>About Robert</h2>
          <p>
      ...
      <section className="aboutRecipeBoxCta">
        ...
        <div className="aboutRecipeBoxCtaButtons">
          <button type="button" className="primary" onClick={() => setActivePage("Recipes")}>Browse Recipes</button>
          <button type="button" className="secondary" onClick={() => setActivePage("Meal Planner")}>Start Meal Planning</button>
        </div>
      </section>
    </main>
  );
    ...
    <article className={`compactDinnerCard${isSelected ? " isSelected" : ""}`}>
      ...
      >
        ...
        <span className="compactDinnerCardCopy">
          ...
          <span className="compactDinnerCardActionRow"><span className="compactDinnerCardAction">{isSelected ? "Details Open" : "View Dinner Details"}</span></span>
        </span>
      </button>
  ...
  const filteredMeals = useMemo(() => {
    return dinnerCombinations
      ...
      .sort((a, b) => {
        ...
        return favoriteDifference || String(a.title || a.mainDish || "").localeCompare(String(b.title || b.mainDish || ""), undefined, { sensitivity: "base" });
      });
  }, [
    calorieRange,
    collectionFilter,
    ...
    <main className="pageShell dinnerCombinationsPage">
      <section className="dinnerCombinationFinder" aria-labelledby="dinnerCombinationFinderTitle">
        <SectionIntro
          ...
          text="Search or choose a category to find a ready-made dinner combination."
        ...
        <div className="dinnerCategorySegmented completeDinnerCategorySegmented" role="group" aria-label="Complete Dinner categories">
          <label className="completeDinnerCategorySearch">
            <input
              type="search"
              value={searchTerm}
              onFocus={() => selectDinnerCategory("all")}
              onChange={(event) => { setSearchTerm(event.target.value); setSelectedDinnerId(""); setVisibleDinnerCount(COMPLETE_DINNER_BATCH_SIZE); }}
              placeholder="Search for..."
              aria-label="Search Complete Dinners"
            />
          </label>
          {[
            ["american", "AMERICAN"],
...
/>
      ...
      {activePage === "About Recipe Box Hidden" && (
        <>
          ...
          />
          <HiddenAboutRecipeBoxPage setActivePage={setActivePage} />
        </>
      )}
          ...
          <article className="foodSafetyGuide" aria-label="Food Safety Guide">
            ...
            <div className="foodSafetyAccordion">
              ...
              <details className="foodSafetyAccordionItem">
                <summary>
                  <span>Clean Hands, Equipment, and Surfaces</span>
                  <span className="foodSafetyAccordionChevron" aria-hidden="true">⌄</span>
                </summary>
...
<p>Do not wash raw chicken, turkey, or other poultry. Splashing water can spread bacteria from the poultry to sinks, counters, utensils, and nearby foods.</p>
              ...
              <details className="foodSafetyAccordionItem">
                <summary>
                  <span>Prevent Cross-Contamination</span>
                  <span className="foodSafetyAccordionChevron" aria-hidden="true">⌄</span>
                </summary>
                <div className="foodSafetyAccordionContent">
...
<p>Use clean utensils when tasting or serving food. Do not reuse a spoon after tasting unless it has been washed.</p>
              ...
              <details className="foodSafetyAccordionItem">
                <summary>
                  <span>Handle Groceries Safely</span>
                  <span className="foodSafetyAccordionChevron" aria-hidden="true">⌄</span>
                </summary>
                <div className="foodSafetyAccordionContent">
...
<p>Food may also be cooked directly from frozen when the cooking method and recipe allow it, although additional cooking time will usually be required. Food should not be thawed or marinated at room temperature.</p>
              ...
              <details className="foodSafetyAccordionItem">
                <summary>
                  <span>Marinate Food Safely</span>
                  <span className="foodSafetyAccordionChevron" aria-hidden="true">⌄</span>
                </summary>
                <div className="foodSafetyAccordionContent">
...
<p>Do not reuse marinade that has touched raw food unless it is first brought to a full boil. A safer option is to reserve a separate portion of unused marinade for brushing or serving.</p>
              ...
              <details className="foodSafetyAccordionItem">
                <summary>
                  <span>Cook Food to a Safe Temperature</span>
                  <span className="foodSafetyAccordionChevron" aria-hidden="true">⌄</span>
                </summary>
                <div className="foodSafetyAccordionContent">
...
<p>Check the temperature in several locations, especially when reheating casseroles, soups, meats, and frozen meals. Microwaved leftovers should reach 165°F.</p>
              ...
              <details className="foodSafetyAccordionItem">
                <summary>
                  <span>Slow Cookers and Warming Appliances</span>
                  <span className="foodSafetyAccordionChevron" aria-hidden="true">⌄</span>
                </summary>
                <div className="foodSafetyAccordionContent">
...
<p>A slow cooker may be used to keep fully cooked food hot, but it should not be used to reheat cold leftovers unless the manufacturer specifically states that it is safe for that purpose. Reheat leftovers to 165°F first, then transfer them to the preheated slow cooker for holding.</p>
              ...
              <details className="foodSafetyAccordionItem">
                <summary>
                  <span>Keep Hot Food Hot and Cold Food Cold</span>
                  <span className="foodSafetyAccordionChevron" aria-hidden="true">⌄</span>
                </summary>
                <div className="foodSafetyAccordionContent">
...
<p>Keep cold food at <strong>40°F or below</strong> by refrigerating it or placing serving dishes over ice.</p>
              ...
              <details className="foodSafetyAccordionItem">
                <summary>
                  <span>Follow the Two-Hour Rule</span>
                  <span className="foodSafetyAccordionChevron" aria-hidden="true">⌄</span>
                </summary>
                <div className="foodSafetyAccordionContent">
...
<p>Avoid tightly packing a warm refrigerator with large containers because cold air needs room to circulate.</p>
              ...
              <details className="foodSafetyAccordionItem">
                <summary>
                  <span>Refrigerator and Freezer Temperatures</span>
                  <span className="foodSafetyAccordionChevron" aria-hidden="true">⌄</span>
                </summary>
                <div className="foodSafetyAccordionContent">
...
<p>Reheat only the amount needed whenever practical. Repeated cycles of warming and cooling reduce quality and increase the chance of unsafe handling.</p>
              ...
              <details className="foodSafetyAccordionItem">
                <summary>
                  <span>General Refrigerator Storage Times</span>
                  <span className="foodSafetyAccordionChevron" aria-hidden="true">⌄</span>
                </summary>
                <div className="foodSafetyAccordionContent">
...
<p>Freezer burn affects quality but does not automatically make food unsafe.</p>
              ...
              <details className="foodSafetyAccordionItem">
                <summary>
                  <span>Safe Serving at Parties and Cookouts</span>
                  <span className="foodSafetyAccordionChevron" aria-hidden="true">⌄</span>
                </summary>
                <div className="foodSafetyAccordionContent">
...
<p>Discard perishable refrigerated food that has remained above 40°F for two hours or longer. Never taste food to test its safety.</p>
              ...
              <details className="foodSafetyAccordionItem">
                <summary>
                  <span>Watch for Signs of Spoilage</span>
                  <span className="foodSafetyAccordionChevron" aria-hidden="true">⌄</span>
                </summary>
                <div className="foodSafetyAccordionContent">
...
<p>Do not taste questionable food. When its safety is uncertain, throw it away.</p>
              ...
              <details className="foodSafetyAccordionItem">
                <summary>
                  <span>People at Greater Risk</span>
                  <span className="foodSafetyAccordionChevron" aria-hidden="true">⌄</span>
                </summary>
                <div className="foodSafetyAccordionContent">
